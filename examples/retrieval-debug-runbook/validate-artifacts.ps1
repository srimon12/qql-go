...
Assert-True ($explain.ok -and $explain.plan.Contains("Search: HYBRID")) "explain plan should stay on the hybrid path"

$hybrid = Read-Json (Join-Path $Artifacts "05-search-hybrid.json")
Assert-True ($hybrid.ok -and $hybrid.data.results[0].id -eq "4104") "hybrid search should rank the billing regression incident first"

$exact = Read-Json (Join-Path $Artifacts "06-search-exact.json")
Assert-True ($exact.ok -and $exact.data.results[0].id -eq "4104") "exact search should rank the billing regression incident first"

$sparse = Read-Json (Join-Path $Artifacts "07-search-sparse.json")
Assert-True ($sparse.ok -and $sparse.data.results[0].id -eq "4104") "sparse search should rank the billing regression incident first"

$filtered = Read-Json (Join-Path $Artifacts "08-search-filtered.json")
Assert-True ($filtered.ok -and $filtered.data.results[0].id -eq "4104") "filtered billing search should keep the regression incident first"
